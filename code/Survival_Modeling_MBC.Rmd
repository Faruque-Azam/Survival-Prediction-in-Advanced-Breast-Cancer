---
title: "Modeling PFS and OS in MBC phase II trials"
author: "Faruque Azam"
date: "2026-01-03"
...
library(tidymodels)
    ...
    initial_split(prop = 0.8, 
                  strata = mOS_2) #change strata to "Subtype" if needed
    ranger_split
...
library(patchwork)
  ...
  set.seed(786)
    ...
    filter(Median_OS < 51) %>% 
    initial_split(prop = 0.8, 
                  strata = Subtype) #change strata to "Median_OS" if needed
...
library(tidymodels)
    ...
    pfs_ranger_split = PFS_df %>% 
      filter(mPFS_2 < 17) %>% 
    initial_split(prop = 0.8, 
                  strata = mPFS_2) #change strata to "Subtype" if needed
    pfs_ranger_split
...
library(patchwork)
  ...
  set.seed(786)
    ...
    initial_split(prop = 0.8, 
                  strata = Median_PFS) # change strata to "Subtype" if needed
...
show_best(pfs_tree_res, metric = "rmse")
show_best(pfs_tree_res, metric = "rsq")
